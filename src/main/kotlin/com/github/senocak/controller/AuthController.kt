    ...
    fun register(
        ...
        val user: User = User()
            .also { it: User ->
                ...
                it.password = passwordEncoder.encode(signUpRequest.password)
                it.roles = arrayListOf(userRole)
            }
            .run { userService.save(user = this) }
            .also { log.info("User created. User: $it") }
        val kafkaMessageTemplate = KafkaMessageTemplate(action = KafkaAction.MAKE_BUCKET, value = "${user.id}")
        bucketCreateRequestKafkaProducer.produce(msg = jacksonObjectMapper.writeValueAsString(kafkaMessageTemplate))
            .takeIf { bucketCreateRequestKafkaProducer.running }
        try {
